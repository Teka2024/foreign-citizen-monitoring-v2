...
// ==================== TRANSFER OPTIONS ====================
// ✅ Returns ALL active accommodations for the transfer picker (bypasses officer scope)
router.get('/transfer-options', authenticate, async (req, res) => {
  try {
    console.log('📊 Fetching transfer-options (all active accommodations)...');

    const accommodations = await Accommodation.find({
      status: 'active'
    }).select('name type address capacity currentOccupants status');

    console.log(`✅ Found ${accommodations.length} accommodations for transfer options`);
...
// ✅ These MUST be before /:id or Express will treat them as ids
router.post('/transfer-request', authenticate, authorize('admin', 'officer'), accommodationController.createTransferRequest);
router.get('/transfer-requests', authenticate, accommodationController.getTransferRequests);
router.post('/transfer-request/:id/accept', authenticate, authorize('admin', 'officer'), accommodationController.acceptTransferRequest);
router.post('/transfer-request/:id/reject', authenticate, authorize('admin', 'officer'), accommodationController.rejectTransferRequest);
router.post('/transfer-request/:id/cancel', authenticate, authorize('admin', 'officer'), accommodationController.cancelTransferRequest);
  ...
  async (req, res) => {
    try {
      ...
      await TransferRequest.updateMany(
        ...
        { status: 'cancelled', rejectionReason: 'Citizen departed the country' }
      );
